...
def evaluate_and_save_encodings(args, model, test_dataset, collate_fn, save_dir):
    ...
    for atom_bond_graphs, bond_angle_graphs, valids, labels in data_gen:
        ...
        # Get predictions and graph encodings
        preds, graph_encodings = model(atom_bond_graphs, bond_angle_graphs, return_graph_encodings=True)
        all_encodings.append(graph_encodings.numpy())
        all_labels.append(labels.numpy())

        predicted = preds.numpy() > 0.5
        true = labels.numpy()
    ...
    # Save the graph encodings to a file
    all_encodings = np.concatenate(all_encodings, axis=0)
    np.save(os.path.join(save_dir, 'graph_encodings.npy'), all_encodings)
...
def main(args):
    ...
    print("encoding the encodings")
    evaluate_and_save_encodings(args, model, dataset, collate_fn, args.model_dir)
