...
def get_dataset(dataset_name, data_path, task_names):
    ...
    elif datast_name == 'knot':
        return load_knot_dataset(data_path, task_names)
    else:
        raise ValueError('%s not supported' % dataset_name)
...
def calc_rocauc_score_multi(labels, preds, valid):
    """Compute ROC-AUC and average across tasks for multi-class multi-label scenario."""
    if labels.ndim == 1:
    ...
    for i in range(labels.shape[1]):
        ...
        unique_labels = np.unique(c_label)

        ula.append(unique_labels)
        # Check if there are at least two unique labels for ROC-AUC computation
        if len(unique_labels) >= 2:
            print("I GOT HERE\n\n\n\n")
            flag = True
            # Compute ROC-AUC for each class
            rocauc_i = roc_auc_score((c_label[:, None] == unique_labels).astype(int), c_pred, average='weighted')
            rocauc_list.append(rocauc_list.append(roc_auc_score(c_label, c_pred)))

    print('Valid ratio: %s' % (np.mean(valid)))
    print('Task evaluated: %s/%s' % (len(rocauc_list), labels.shape[1]))
    if len(rocauc_list) == 0:
        raise RuntimeError(f"No positively labeled data available. Cannot compute ROC-AUC Flag was: {flag}. unique_labels array was {ula}")
...
def custom_multi_label_roc_auc(labels, predictions):
    """
    Calculate the ROC-AUC score for multi-label classification.
    
    Args:
    - labels (paddle.Tensor): The true labels, shape (n_samples, n_labels)
    - predictions (paddle.Tensor): The predicted probabilities, shape (n_samples, n_labels)
    
    Returns:
    - float: The averaged ROC-AUC score across all labels
    """
    ...
    for label_idx in range(n_labels):
        # Extract the current label's true labels and predictions
        true_label = labels[:, label_idx]
        pred_label = predictions[:, label_idx]

        # Calculate ROC-AUC for the current label
    ...
    # Average the AUC scores across all labels
