...
class DownstreamTransformFnPDB(object):
    ...
    def __call__(self, raw_data):
        ...
        if not self.is_inference:
            data['label'] = np.array(raw_data['labels']).reshape([-1])
    ...
    def __call__(self, data_list):
        ...
        if not self.is_inference:
            if self.task_type == 'class':
                ...
                # label: -1 -> 0, 1 -> 1 jk were not doin this
                labels = labels
                valids = (((labels + 1.0) / 2) != 0.5)
...
if __name__ == '__main__':
    print(DownstreamTransformFnPDB()('/gpfs/gibbs/pi/gerstein/as4272/KnotFun/data_files/1by7/1by7_rec.pdb', []))
